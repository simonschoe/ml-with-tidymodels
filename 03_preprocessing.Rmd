...
.footnote[
*Note: Some functions internally convert a data frame to a numeric design matrix (e.g., `lm()` automatically one-hot encodes unordered factors or creates polynomial contrasts from ordered factors).*
]
...
.pull-right[.pull-right[.footnote[
*Note: Each* `step_*()` *function contains a `skip` argument which is mostly equal to `FALSE` by default. Yet, for certain preprocessing steps (e.g., under- or oversampling) we want to ensure that the step is not applied to the test set to retain the original properties of the unseen test data.*
]]]
