...
- Use `step_other()` to lump together rarely occurring factor levels. `peak_name`, `citizenship` and `expedition_role` all have several 100 factor levels and hence a high risk of being near-zero variance features. All factor levels with a relative frequency below 5% are pooled into `"other"`.
...
4. Evaluate the model on the test by using the same mean and standard deviation computed in the first step to normalize test set observations. 
]
]
...
- smote worked (72,369 vs. 61,177 samples in the original train_set)
