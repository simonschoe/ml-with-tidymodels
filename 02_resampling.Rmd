...
???
- `repeats` enables repetition of the basic CV scheme, e.g., with `repeats=3`, there will be 30 folds in total -> to produce an even more robust estimate of your test set error that is independent of the way the folds were resampled

---

## 4.1 `rsample`: Resampling Infrastructure

To extract the training and validation data from each fold, we use the `anaylsis()` and `assessment()` functions:
```{r, eval=F}
climbers_folds %>%
  pluck("splits", 1) %>%
  analysis()
...
.footnote[
<i>Note: For data with a strong time-series component (i.e. temporally ordered data), `rsample` provides an appropriate resampling infrastructure as well. For example, you may use `initial_time_split()` to produce a non-random temporal train-test-split and `rolling_origin()` or the `slide_*()` methods to generate time-series resamples.</i>
]
